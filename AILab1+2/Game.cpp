...
Game::Game() :
	...
	m_slowArriveShip{ sf::Vector2f{400, 500}, "slowArriveShip", "ArriveSlow", &m_ship, 100.0, 50.0 },
	m_fastArriveShip{ sf::Vector2f{500, 500}, "fastArriveShip", "ArriveFast", &m_ship, 400.0, 100.0 },
	m_exitGame{ false } //when true game will exit
{
	m_font.loadFromFile("ASSETS\\FONTS\\ariblk.ttf");
	m_seekShip.setFont(m_font);
	m_wanderShip.setFont(m_font);
	m_slowArriveShip.setFont(m_font);
	m_fastArriveShip.setFont(m_font);

	m_seekShip.setBehaviour(AIStates::SEEK);
	m_wanderShip.setBehaviour(AIStates::WANDER);
	m_slowArriveShip.setBehaviour(AIStates::ARRIVE);
...
void Game::processKeys(sf::Event t_event)
{
	...
	m_slowArriveShip.handleEvents(t_event);
...
void Game::update(sf::Time t_deltaTime)
{
	...
	m_slowArriveShip.update(t_deltaTime);
...
void Game::render()
{
	...
	m_slowArriveShip.render(m_window);
